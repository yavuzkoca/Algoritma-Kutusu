...
public:
    CsvOp(char inCruncher): cruncher(inCruncher) {}
    CsvOp(char inCruncher, std::string inFileName);
    CsvOp(std::vector<std::vector<std::string>> inCsvTable): csvTable(inCsvTable) {}
    ~CsvOp();
    ...
    inline void insert(int lineOrder, std::vector<std::string> line)
